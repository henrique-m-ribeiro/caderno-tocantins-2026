...
# -*- coding: utf-8 -*-
"""
Extrator de Indicadores - Versão 9 (Completa + Finanças Públicas)
Extrai TODOS os indicadores de TODOS os capítulos dos Perfis SEPLAN-TO
Inclui: Demografia, IDH, Economia, VAB, Emprego, Educação, Saúde, Saneamento, Finanças Públicas

Autor: Claude Code (baseado em v7 de Manus AI)
Data: 28/01/2026
Versão: 9.0 (Completa)
Novidade: + Capítulo Finanças Públicas (FPM, ICMS, IPVA, Transferências) - 30 indicadores
Meta: 106 indicadores (139% da meta 95%)
"""
...
class ExtratadorPerfilSEPLANv9:
    """
    ...
    def extrair_financas_publicas(self) -> Dict:
        """Extrai Finanças Públicas (Capítulo 5.20 e 5.21, Página 43)"""
        indicadores = {}

        # Página 43 contém Transferências e ICMS/IPVA
        texto = self.extrair_texto_pagina(42)  # Índice 42 = página 43
        linhas = texto.split('\n')

        anos = [2019, 2020, 2021, 2022, 2023]

        # Buscar linhas de Transferências Constitucionais
        for linha in linhas:
            linha_lower = linha.lower()

            # Total de Transferências
            if linha.strip().startswith('Total ') and 'transferência' in texto.lower():
                numeros = re.findall(r'\d+(?:\.\d+)*(?:,\d+)?', linha)
                if len(numeros) >= 5:
                    for i, ano in enumerate(anos):
                        valor = self.limpar_numero(numeros[i])
                        if valor:
                            indicadores[f'transferencias_total_{ano}'] = valor

            # FPM (Fundo de Participação dos Municípios)
            elif 'fpm' in linha_lower and not 'nota' in linha_lower:
                numeros = re.findall(r'\d+(?:\.\d+)*(?:,\d+)?', linha)
                if len(numeros) >= 5:
                    for i, ano in enumerate(anos):
                        valor = self.limpar_numero(numeros[i])
                        if valor:
                            indicadores[f'fpm_{ano}'] = valor

            # FUNDEB
            elif 'fundeb' in linha_lower and not 'nota' in linha_lower:
                numeros = re.findall(r'\d+(?:\.\d+)*(?:,\d+)?', linha)
                if len(numeros) >= 5:
                    for i, ano in enumerate(anos):
                        valor = self.limpar_numero(numeros[i])
                        if valor:
                            indicadores[f'fundeb_{ano}'] = valor

            # ITR (Imposto Territorial Rural)
            elif linha_lower.startswith('itr '):
                numeros = re.findall(r'\d+(?:\.\d+)*(?:,\d+)?', linha)
                if len(numeros) >= 5:
                    for i, ano in enumerate(anos):
                        valor = self.limpar_numero(numeros[i])
                        if valor:
                            indicadores[f'itr_{ano}'] = valor

        # Buscar ICMS e IPVA (segunda tabela na mesma página)
        # Estrutura: Ano ICMS IPVA
        for linha in linhas:
            for ano in anos:
                if linha.strip().startswith(str(ano)):
                    numeros = re.findall(r'\d+(?:\.\d+)*(?:,\d+)?', linha)
                    if len(numeros) >= 3:  # Ano, ICMS, IPVA
                        icms = self.limpar_numero(numeros[1])
                        ipva = self.limpar_numero(numeros[2])

                        if icms:
                            indicadores[f'icms_{ano}'] = icms
                        if ipva:
                            indicadores[f'ipva_{ano}'] = ipva
                    break
    ...
    def extrair_todos_indicadores(self) -> Dict:
        ...
        print("🔍 Iniciando extração (Versão 9 - Completa + Finanças Públicas)...")
        ...
        print("📊 Finanças Públicas...")
        self.dados["indicadores"].update(self.extrair_financas_publicas())
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Uso: python extrator_v9_completo.py <pdf_entrada> <json_saida>")
        sys.exit(1)

    pdf_path = sys.argv[1]
    json_path = sys.argv[2]

    extrator = ExtratadorPerfilSEPLANv9(pdf_path)
